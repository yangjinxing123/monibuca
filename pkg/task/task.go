...
func (task *Task) Stop(err error) {
	...
	task.stopOnce.Do(func() {
		if task.CancelCauseFunc != nil {
			task.Debug("task stop", "caller", fmt.Sprintf("%s:%d", strings.TrimPrefix(file, sourceFilePathPrefix), line), "reason", err, "elapsed", time.Since(task.StartTime), "taskId", task.ID, "taskType", task.GetTaskType(), "ownerType", task.GetOwnerType())

...
func (d *Device) onMessage(req *sip.Request, tx sip.ServerTransaction, msg *gb28181.Message) (err error) {
	d.plugin.Trace("into onMessage", "deviceid", d.DeviceId)
	source := req.Source()
	...
	case "Keepalive":
		...
		d.Debug("into keepalive", "deviceid", d.DeviceId, "keepaliveTime", d.KeepaliveTime, "keepaliveInterval", d.KeepaliveInterval)
		if d.plugin.DB != nil {
	...
	case "Catalog":
		...
		d.Debug("save channel", "deviceid", d.DeviceId, "channels_length", d.channels.Length, "channel_count", d.ChannelCount, "update_time", d.UpdateTime)

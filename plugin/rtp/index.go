...
func (p *RTPPlugin) ReceivePS(ctx context.Context, req *pb.ReceivePSRequest) (resp *pb.ReceivePSResponse, err error) {
	...
	if err != nil {
		...
		p.Error("publish stream for rtp", "error", err)
		return resp, err
	}
	...
	if req.Udp {
		...
		writeRTP = func() (err error) {
			...
			for r.Length > 0 {
				...
				buf.Reset()
				buf.PushOne(headerBuf[:12])
				_, err = packet.Header.MarshalTo(headerBuf[:12])
				if err != nil {
					return
				}
				r.RangeN(mrtp.MTUSize, buf.PushOne)
				n, _ := buf.WriteTo(w)
		...
		writeRTP = func() (err error) {
			defer mem.Recycle()
			buf.Reset()
			buf.PushOne(headerBuf[:14])
			packet.Timestamp = uint32(time.Now().UnixMilli()) * 90
			binary.BigEndian.PutUint16(headerBuf[:2], uint16(mem.Size+12))
			packet.SequenceNumber += 1
			_, err = packet.Header.MarshalTo(headerBuf[2:14])
			if err != nil {
				return
			}
			buf.Push(mem.Buffers...)
			n, _ := buf.WriteTo(w)
			totalBytesSent += int(n)
			return
		}

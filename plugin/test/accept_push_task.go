...
func (rt *AcceptPushTask) Start() error {
	...
	case "srt":
		args = append(args,
			"-f", "mpegts",
			fmt.Sprintf("srt://%s?streamid=publish:/%s", rt.ServerAddr, rt.StreamPath),
		)
	case "ps":
		host := rt.ServerAddr
	...
	rt.testCase.Logger.Info("FFmpeg command", "command", strings.Join(args, " "))
...
func (rt *AcceptPushTask) Dispose() {
	rt.testCase.Dispose()
	if rt.process != nil {
		rt.process.Kill()
	}

...
func (st *SnapshotTask) Run() error {
	...
	if st.Format == "" {
		...
		subscriber, err := st.testCase.Plugin.SubscribeWithConfig(context.Background(), streamPath, subConfig)
		if err != nil {
	...
	st.testCase.Logger.Info("Snapshot completed successfully", "outputSize", buf.Len())

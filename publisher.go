...
func (p *PublishNoDataTimeout) Tick(any) {
	...
	if p.Publisher.PubVideo && p.Publisher.VideoTrack.CheckTimeout(p.Publisher.PublishTimeout) {
		p.Error("video timeout", "writeTime", p.Publisher.VideoTrack.LastValue.WriteTime)
		if !p.Publisher.HasAudioTrack() {
			p.Publisher.Stop(ErrPublishTimeout)
		}
		p.Publisher.NoVideo()
	}
	if p.Publisher.PubAudio && p.Publisher.AudioTrack.CheckTimeout(p.Publisher.PublishTimeout) {
		p.Error("audio timeout", "writeTime", p.Publisher.AudioTrack.LastValue.WriteTime)
		if !p.Publisher.HasVideoTrack() {
			p.Publisher.Stop(ErrPublishTimeout)
		}
		p.Publisher.NoAudio()
	}
...
func (p *Publisher) trackAdded() {
	if p.State == PublisherStateInit && p.PublishTimeout > 0 {
